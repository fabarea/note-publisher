...
const baseSourcePath = 'Sync/Notes/public'
  ...
  args.sourceNotes.map(fileName => {
    ...
    unified()
      ...
      .process(vfile.readSync(sourceNoteFileNameAndPath), (err, file) => {
        ...
        if (rawMd5Value !== args.dataSource[sourceNoteFileNameAndPath]) {
          let content = processFrontMatter(rawContent, sourceNoteFileNameAndPath)
          ...
          // Store
          args.dataSource[sourceNoteFileNameAndPath] = checksum(content)
...
function saveDataSource(args = []) {
  fs.writeFileSync(args.dataSourcePath, JSON.stringify(args.dataSource), err => {
    if (err) throw err
    console.log("It's saved!")
  })
...
function processFrontMatter(content, sourceNoteFileNameAndPath) {
  const newFrontMatter = getUpdatedFrontMatter(content, sourceNoteFileNameAndPath)
...
function getFilesFromPath(path, extension) {
  ...
  return directory.filter(file => file.match(new RegExp(`.*\.(${extension})`, 'ig')))
}
...
function getUpdatedFrontMatter(content, fileName) {
  ...
  // ast.attributes.date = getModifiedTime(fileName)
  // ast.attributes.timeStamp = modifiedTime.getTime();
  delete ast.attributes.date
